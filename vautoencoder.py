...
def generate_and_save_images(model, epoch, test_sample):
  ...
  predictions = np.where(predictions > .5, 1.0, 0.0)
...
for test_batch in test_dataset.take(1):
  test_sample = np.copy(test_batch[0:num_examples_to_generate, :, :, :])
  j=0
  for i in range(test_images.shape[0]):
    if test_categories[i] == j:
        test_sample[j] = test_images[i]
        j += 1
        if j == 10:
            j = 0
...
for epoch in range(1, epochs + 1):
  ...
  if epoch % 10 == 9:
    model.encoder.save(f'encoder{epoch}.h5')
    model.decoder.save(f'decoder{epoch}.h5')
...
with imageio.get_writer(anim_file, mode='I') as writer:
  filenames = glob.glob('image_at*.png')
